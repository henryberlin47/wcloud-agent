...
export async function ensureRclone(helpers) {
  ...
  const probe = await run(helpers, 'rclone', ['version'], { quiet: true, timeout: 10_000 });
  if (probe.code === 0) {
    rcloneChecked = true;
    return;
  }
  ...
  const again = await run(helpers, 'rclone', ['version'], { quiet: true, timeout: 10_000 });
  if (inst.code !== 0 || again.code !== 0) {
