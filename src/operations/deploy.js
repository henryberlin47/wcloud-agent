...
export async function runDeploy(job, helpers, p) {
  ...
  await createSite(helpers, {
    domain, type: p.type, php: p.php, cache: p.cache, enableWww: p.enableWww, canonical: p.canonical,
    wp: { adminUser: p.wp_user, adminPassword: p.wp_password },
  });
  ok(`Site created — ${domain}`);

  if (p.type === 'wordpress') {
    // Not fatal: the site works without them; they can be turned on later.
